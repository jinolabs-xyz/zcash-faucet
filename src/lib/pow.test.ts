...
test("issueChallenge: shape, expiry window, and a signature we can reproduce", () => {
...
test("accepts a correctly solved challenge", () => {
  ...
  assert.deepEqual(verifySolution({ ...ch, nonce }, IP), { ok: true });
});
...
test("rejects a tampered signature", () => {
  ...
  const v = verifySolution({ ...ch, sig, nonce }, IP);
  assert.equal(v.ok, false);
  assert.match(v.reason ?? "", /signature/i);
});

test("rejects a solution presented from a different ipHash", () => {
  const ch = issueChallenge(IP);
  const nonce = findNonce(ch.seed, ch.difficulty);
  // The sig binds the ipHash, so a stolen solution fails the signature check.
  const v = verifySolution({ ...ch, nonce }, "iphash-someone-else");
  assert.equal(v.ok, false);
  assert.match(v.reason ?? "", /signature/i);
});
...
test("rejects an expired challenge", () => {
  ...
  const v = verifySolution({ ...ch, nonce: findNonce(seed, 8) }, IP);
  assert.equal(v.ok, false);
  assert.match(v.reason ?? "", /expired/i);
});

test("rejects difficulty outside the configured bounds", () => {
  ...
  for (const difficulty of [6, 27]) {
    ...
    const v = verifySolution({ ...ch, nonce: "0" }, IP);
    assert.equal(v.ok, false, `difficulty ${difficulty} should be rejected`);
    assert.match(v.reason ?? "", /difficulty/i);
  }
});

test("rejects a nonce that misses the required leading zero bits", () => {
  const ch = issueChallenge(IP);
  const nonce = findNonce(ch.seed, ch.difficulty, false);
  const v = verifySolution({ ...ch, nonce }, IP);
  assert.equal(v.ok, false);
  assert.match(v.reason ?? "", /does not meet/i);
});

test("rejects a replayed challenge: each sig is single-use", () => {
  const ch = issueChallenge(IP);
  const nonce = findNonce(ch.seed, ch.difficulty);
  assert.equal(verifySolution({ ...ch, nonce }, IP).ok, true);
  const again = verifySolution({ ...ch, nonce }, IP);
  assert.equal(again.ok, false);
  assert.match(again.reason ?? "", /already used/i);
});

test("rejects a missing or fieldless solution", () => {
  // @ts-expect-error deliberately malformed
  assert.equal(verifySolution({}, IP).ok, false);
  const ch = issueChallenge(IP);
  // @ts-expect-error nonce absent
  assert.equal(verifySolution({ ...ch }, IP).ok, false);
});
