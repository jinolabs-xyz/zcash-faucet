...
test("a hammering client escalates per request and hits the maxBits ceiling", () => {
  const seen: number[] = [];
  for (let i = 0; i < 5; i++) {
...
test("someone else's hammering does not escalate a fresh client", () => {
...
test("global pressure raises difficulty for brand-new clients", () => {
...
test("a burst of solved challenges all verify once and none replay", () => {
  ...
  for (const { ipHash, sol } of solved) {
    assert.deepEqual(verifySolution(sol, ipHash), { ok: true });
  }
  for (const { ipHash, sol } of solved) {
    const again = verifySolution(sol, ipHash);
    assert.equal(again.ok, false);
    assert.match(again.reason ?? "", /already used/i);
  }
});

test("a replayed sig is refused even with a different valid nonce", () => {
  const ipHash = "iphash-renonce";
  const ch = issueChallenge(ipHash);
  globalCount++;
  const first = solveNonce(ch.seed, ch.difficulty);
  assert.equal(verifySolution({ ...ch, nonce: first }, ipHash).ok, true);
  ...
  const v = verifySolution({ ...ch, nonce: second }, ipHash);
  assert.equal(v.ok, false);
  assert.match(v.reason ?? "", /already used/i);
});

test("expired challenges are refused in bulk, solved or not", () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const v = verifySolution(sol, ipHash);
