...
check "the balance lookup takes the address in a POST body, and the page sends it that way" \
  "grep -q 'export const POST = withApi(\"balance\"' '$REPO/src/app/api/balance/route.ts' && ! grep -q 'searchParams.get(\"address\")' '$REPO/src/app/api/balance/route.ts' && ! grep -q '/api/balance?address=' '$REPO/src/app/page.tsx'"
