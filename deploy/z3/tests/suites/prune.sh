...
check "unused tagged images are LISTED for a human" "grep -q 'held by no container (left alone' '$T/out.log' && grep -q 'zfnd/zebra:6.2.0' '$T/out.log'"
check "and the list excludes what a container holds and the dangling entries" "! grep 'held by no container' '$T/out.log' | grep -qE 'zfnd/zebra:6.3.0|zcash-faucet:latest|<none>'"
...
check "exits 0" "[ \"\$(cat '$T/rc')\" = 0 ]"
check "the build prune is described" "grep -q 'DRY RUN: docker builder prune -af --reserved-space 5GB' '$T/dry.log'"
check "the dangling prune is described" "grep -q 'DRY RUN: docker image prune -f' '$T/dry.log'"
