...
# AND AT LEAST 2.24. The generated override below binds the RPC ports with `ports: !override`,
# which REPLACES the upstream list; a compose too old for the tag ignores it and MERGES, and a
# merged list carries the upstream's 0.0.0.0 binding beside ours - the wallet RPC open to the
# internet with no error printed, which is exactly the hole this is closing (2026-09-21, a
# month of exposure). So an old compose is refused, not worked around.
compose_ver="$(docker compose version --short 2>/dev/null | sed -E 's/^v//; s/[^0-9.].*$//' \
  | awk -F. 'NF>0 && $1 != "" { printf "%d.%d.%d\n", $1, ($2 == "" ? 0 : $2), ($3 == "" ? 0 : $3) }')"
compose_at_least() { # $1 have, $2 want: dotted versions
  [ "$(printf '%s\n%s\n' "$2" "$1" | sort -t. -k1,1n -k2,2n -k3,3n | head -n1)" = "$2" ]
}
if [ -z "$compose_ver" ]; then
  echo "Cannot read the Docker Compose version ('docker compose version --short' printed nothing usable). 'ports: !override' needs 2.24 or newer and an older compose would leave the RPC ports on 0.0.0.0, so this does not guess. Not continuing."
  exit 1
fi
if ! compose_at_least "$compose_ver" "2.24.0"; then
  echo "Need Docker Compose 2.24 or newer for 'ports: !override' (found $compose_ver): an older one merges port lists and leaves the RPC ports on 0.0.0.0. Not continuing."
  exit 1
fi
...
# have quietly unfunded the box. Caught by generating the file and reading it.
  ...
  # ALWAYS WRITTEN NOW, not only when there is an address or a pin to carry: the override is
  # where the RPC ports get bound to loopback, and a box with neither still needs that. The
  # shell env in Z3_LOOPBACK_BINDINGS only reaches the compose call THIS script makes; a hand
  # run of `docker compose up` in the clone - the 2026-08-30 upgrade was one - reads the
  # upstream defaults and publishes zebra's RPC, its health port and the WALLET RPC on 0.0.0.0,
  # where docker's own iptables chain carries them past ufw. The audit logged that 34 times
  # over a month and the fix each time was a hand-run rule that did not survive. A file the
  # stack reads on every invocation is the control; `!override` makes compose REPLACE the
  # upstream port list rather than merge into it (merging would keep the 0.0.0.0 entry beside
  # ours). P2P stays on all interfaces on purpose: inbound peers are the point of it.
  rpc_port=$([ "$NETWORK" = testnet ] && echo 18232 || echo 8232)
  p2p_port=$([ "$NETWORK" = testnet ] && echo 18233 || echo 8233)
  health_port=$([ "$NETWORK" = testnet ] && echo 18080 || echo 8080)
  zallet_port=$([ "$NETWORK" = testnet ] && echo 40232 || echo 28232)
  zaino_grpc=$([ "$NETWORK" = testnet ] && echo 18137 || echo 8137)
  zaino_json=$([ "$NETWORK" = testnet ] && echo 18237 || echo 8237)
  {
    echo "# GENERATED by deploy/deploy.sh from FAUCET_MINER_ADDRESS, z3/stack-versions.env and the loopback bindings."
    echo "# Do not hand-edit: the next deploy overwrites this file. Change the inputs instead."
    echo "# The ports blocks REPLACE upstream's (!override): RPC-class ports on 127.0.0.1, P2P public."
    echo "services:"
    echo "  zebra:"
    ...
    echo "    ports: !override"
    echo "      - \"127.0.0.1:$rpc_port:$rpc_port\""
    echo "      - \"$p2p_port:$p2p_port\""
    echo "      - \"127.0.0.1:$health_port:8080\""
    echo "  zallet:"
    [ -n "$ZALLET_IMG" ] && echo "    image: \"$ZALLET_IMG\""
    echo "    ports: !override"
    echo "      - \"127.0.0.1:$zallet_port:28232\""
    echo "  zaino:"
    [ -n "$ZAINO_IMG" ]  && echo "    image: \"$ZAINO_IMG\""
    echo "    ports: !override"
    echo "      - \"127.0.0.1:$zaino_grpc:8137\""
    echo "      - \"127.0.0.1:$zaino_json:8237\""
  } > "$OVERRIDE"
fi
