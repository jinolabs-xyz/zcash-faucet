...
| `TURNSTILE_SECRET_KEY` | none | Turnstile server key. Setting it flips the default gate to `turnstile`. Without it, `turnstile` refuses to boot, and the verifier refuses every token: it no longer skips verification. The siteverify call is bounded at 5 s and a slow answer is a refusal. |
